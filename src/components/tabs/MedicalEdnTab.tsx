...
export const MedicalEdnTab: React.FC<MedicalEdnTabProps> = ({
  ...
  return (
    <div className="space-y-7">
      {/* Top Banner with PRSB standard or Primary Care consultation notification */}
      <div className="flex flex-wrap items-center justify-between gap-3 p-4 rounded-2xl bg-[#f0f4f9] border border-blue-100">
        <div className="flex items-center space-x-2.5">
          <div className="w-8 h-8 rounded-xl bg-blue-100/80 border border-blue-200 flex items-center justify-center text-blue-700">
            {isPrimaryCare ? <ClipboardCheck className="w-4 h-4" /> : <Stethoscope className="w-4 h-4" />}
          </div>
          <div>
            <span className="text-xs font-semibold text-slate-800 block">
              {isPrimaryCare
                ? 'NHS General Practice Consultation Note & Management Plan'
                : 'PRSB Standard Electronic Discharge Notification (eDN)'}
            </span>
            <span className="text-[11px] text-slate-500">
              {isPrimaryCare
                ? `Structured coded encounter note formatted for ${summary.patient.systemOrigin} GP electronic health record`
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-2xs">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500">
            {isPrimaryCare ? 'Primary Consultation Diagnosis / Impairment' : 'Primary Discharge Diagnosis'}
          </h3>
      ...
      {/* Hospital Course / Primary Care Systems Plan */}
      <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-2xs">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-4">
              ...
              {isEditing ? (
                ...
                <div className="space-y-1.5 pt-0.5">
                  {item.summary.split('\n').filter((l) => l.trim().length > 0).map((line, lIdx) => {
                    ...
                    return (
                      <div key={lIdx} className="flex items-start space-x-2 text-xs sm:text-sm text-slate-700">
                        <span className="w-1.5 h-1.5 rounded-xs bg-blue-600 mt-2 flex-shrink-0" />
